...
function init_search_page(getdata) {
    ...
    $(".spaces_box_type.add_search,.spaces_box_type.add_socialgroup,.spaces_box_type.add_mystorage").each(function () {
        ...
        p.click(function () {
            ...
            let pr_datas_num =  $(".proo_main_create div.selected_step").find('span').text() - 1;
            data.content = temp_arrayData;
            pr[pr_datas_num].content = data.content;
        })
    });
}
...
/*加载宏页面*/
function init_hong_page(getdata) {
    let hont_html = "";

    hont_html = '<li>\
                            <img src="images/icon-hong.png">\
                            <span>git宏</span>\
                            <i></i>\
                            </li>';

    $('.load_hong').html(hont_html);


    /*创建选择宏页面*/
    $(".spaces_box_type.add_hong").each(function () {
        ...
        p.click(function () {
            ...
            hongorder[0] = "node -v";
            hongorder[1] = "npm -v";
            hongorder[2] = "ipconfig";
            hongorder[3] = "cd /d E:/git/workTRU/TRUIProcess/";
            hongorder[4] = "ls";
           /* hongorder[5] = "\"C:\\Program Files (x86)\\Git\\bin\\git.exe\"";*/

            hong.name = tool_name;
            hong.img = tool_img;
            hong.content = hongorder;

            if (!!$(this).hasClass("selected")) {
                $(this).removeClass("selected");
                removeByValue(temp_arrayData, hong.name);
            } else {
                $(this).addClass("selected");
                temp_arrayData.push(hong);
            }
...
$('#add_type_step li').click(function () {
    ...
    if (!is_mid_Creater) {
        ...
        $(".proo_main_create").append('<div onclick="update_step('+order_step+','+id_num+')" id="pro_step_id_'+ order_step +'" class="proo_box_cont proo_box_cont_' + id_num + ' selected_step">' +
        ...
        for (let i = 0; i < pr_tem.length; i++) {
            sing_h += '<div onclick="update_step('+pr_tem[i].num+','+pr_tem[i].type+')" id="pro_step_id_'+ pr_tem[i].num +'" class="proo_box_cont proo_box_cont_' + pr_tem[i].type + '">' +
...
function update_step(order_step,id_num) {
    ...
    if ($('.leixing').hasClass("to-block") && data.content == "") {
        $.MsgBox.Alert('温馨提示','请先完成此次步骤再添加！');
        return;
    } else {
        clear();
    ...
    if(id_num == 1){
        ...
        var toolname;
        for(let i= 0; i<pr[order_step - 1].content.length; i++){
            toolname = pr[order_step - 1].content[i].name;
            $(".load_had_tool li[cfgName='"+toolname+"']").addClass('selected');
            temp_arrayData.push(pr[order_step - 1].content[i]);
        }
    }
}
/*************************************************点击已创建的步骤，能回显内容并可编辑**************************************/
...
$('.save_img').click(function () {
    ...
    if (($('.leixing').hasClass("to-block") && data.content != "") || (!$('.leixing').hasClass("to-block") && pr != ""))  {
        chanellei_addsave();
    } else {
